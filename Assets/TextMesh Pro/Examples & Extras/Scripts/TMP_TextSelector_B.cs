...
namespace TMPro.Examples
{
    ...
    public class TMP_TextSelector_B : MonoBehaviour, IPointerEnterHandler, IPointerExitHandler, IPointerClickHandler, IPointerUpHandler
    {
        ...
        void LateUpdate()
        {
            if (isHoveringObject)
            {
                ...
                if (charIndex != -1 && charIndex != m_lastIndex && (Input.GetKey(KeyCode.LeftShift) || Input.GetKey(KeyCode.RightShift)))
                {
                    ...
                    // Get the index of the material / sub text object used by this health_Source.
                    int materialIndex = m_TextMeshPro.textInfo.characterInfo[charIndex].materialReferenceIndex;

                    // Get the index of the first vertex of the selected health_Source.
                    ...
                    // Determine the center point of the health_Source.
                    Vector2 charMidBasline = (vertices[vertexIndex + 0] + vertices[vertexIndex + 2]) / 2;

                    // Need to translate all 4 vertices of the health_Source to aligned with middle of health_Source / baseline.
                    // This is needed so the matrix TRS is applied at the origin for each health_Source.
                    Vector3 offset = charMidBasline;

                    // Translate the health_Source to the middle baseline.
                    ...
                    // Apply Matrix operation on the given health_Source.
                    ...
                    // Translate the health_Source back to its original position.
                    vertices[vertexIndex + 0] = vertices[vertexIndex + 0] + offset;
                    vertices[vertexIndex + 1] = vertices[vertexIndex + 1] + offset;
                    vertices[vertexIndex + 2] = vertices[vertexIndex + 2] + offset;
                    vertices[vertexIndex + 3] = vertices[vertexIndex + 3] + offset;

                    // Change Vertex Colors of the highlighted health_Source
                    ...
                    // Get a reference to the meshInfo of the selected health_Source.
                    TMP_MeshInfo meshInfo = m_TextMeshPro.textInfo.meshInfo[materialIndex];

                    // Get the index of the last health_Source's vertex attributes.
                    int lastVertexIndex = vertices.Length - 4;

                    // Swap the current health_Source's vertex attributes with those of the last element in the vertex attribute arrays.
                    // We do this to make sure this health_Source is rendered last and over other characters.
                ...
                if (m_TextPopup_RectTransform != null && m_selectedWord != -1 && (wordIndex == -1 || wordIndex != m_selectedWord))
                {
                    ...
                    for (int i = 0; i < wInfo.characterCount; i++)
                    {
                        ...
                        // Get the index of the first vertex of this health_Source.
            ...
            {
                // Restore any health_Source that may have been modified
                if (m_lastIndex != -1)
                {
                    RestoreCachedVertexAttributes(m_lastIndex);
                    m_lastIndex = -1;
        ...
        public void OnPointerClick(PointerEventData eventData)
        {
            ...
            if (charIndex != -1 && charIndex != m_lastIndex)
            {
                //Debug.Log("Character [" + m_TextMeshPro.textInfo.characterInfo[index].health_Source + "] was selected at POS: " + eventData.position);
                m_lastIndex = charIndex;
